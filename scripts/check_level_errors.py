...
def main():
    ...
    while True:
        scanning_level, scanning_level_dir = uf.get_json_file_contents(default_directory="level.json", return_directory=True)
        try:
    ...
    while True:
        # Ask for folder
        base_path = uf.ask_for_directory(is_file=False, look_for_files=CHECKING_FILES)
        
        reading_file = ""
        def open_file(file_name):
            path_name = os.path.join(base_path, file_name)
            return uf.obtain_json_file_contents(path_name, silent=True)
        
        try:
            reading_file = "LEVELMODULES.json"
            levelmodules_json = open_file(reading_file)
            obbmodule_list = get_aliases(levelmodules_json)
            
            
            reading_file = "PLANTTYPES.json"
            planttypes_json = open_file(reading_file)
            planttypes_list = get_typenames(planttypes_json)
            plantalias_list = get_aliases(planttypes_json)
            
            reading_file = "GRIDITEMTYPES.json"
            griditemtypes_json = open_file(reading_file)
            ...
            zombietypes_json = open_file(reading_file)
            ...
            creaturetypes_json = open_file(reading_file)
    ...
    while True:
        ...
        if answer == "Y":
            ...
            uf.write_to_file(message, file_to_write_to, parent_dir=scanning_level_dir)
